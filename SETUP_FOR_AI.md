...
**`cli-local` with Codex + OpenAI** (ChatGPT-account OAuth, no API key):
...
runtime:
  ...
  model: gpt-5.5
  permission_mode: bypassPermissions
  # api_key not needed; codex shares the host's ~/.codex/auth.json
...
| Codex agent: `mcp__codex_apps__*` (Drive/Gmail/…) reads work but writes fail `ACCESS_TOKEN_SCOPE_INSUFFICIENT` + `TRIGGER_REAUTHENTICATION` | Codex Apps connectors are codex-internal, NOT puffo-managed MCP — `list_mcp_servers` won't show them and `sync_host_mcp` can't fix them. The connector grant is account-level and its write-scope consent is interactive-only: disconnect + reconnect the connector in **interactive** codex (desktop/TUI settings), approve the write scopes, then `refresh(host_sync=True)` on the agent (cli-docker also needs `session=True`) and allow one worker turn (a transient `-32603` during the token transition is expected). See PUF-357. |
