...
def write_codex_mcp_config(
    ...
    *,
    ...
    """Serialise the codex MCP config to ``dest`` as TOML.

    codex reads ``$CODEX_HOME/config.toml`` on startup and treats
    ``[mcp_servers.<name>]`` tables as stdio MCP server entries with
    fields ``command``, ``args`` (array), and ``env`` (table). Phase 0
    #5 verifies the ``env`` table is actually honoured per-server (we
    rely on it for the ``PYTHONUSERBASE`` / ``PUFFO_*`` env vars the
    puffo_core MCP needs).
    ...
    lines: list[str] = [
        ...
        "args = ["
        + ", ".join(f'"{_toml_escape(a)}"' for a in args)
        + "]",
    ]
    if env:
        lines.append("")
