"""Individual cloud lifecycle MCP registration helpers."""
...
def register_schedule_wake_tool(mcp: FastMCP, cfg: Any) -> None:
    @mcp.tool()
    async def schedule_wake(
        ...
        if after_seconds in (None, "", 0, "0"):
            ...
            try:
                ...
                return (
                    "schedule_wake: after_seconds must be an integer number of seconds."
                )
        has_after = after_val > 0
        has_at = bool(wake_at and wake_at.strip())
        if has_after and has_at:
            return (
                "schedule_wake: pass exactly one of after_seconds or wake_at, not both."
            )
...
def register_cancel_wake_tool(mcp: FastMCP, cfg: Any) -> None:
    @mcp.tool()
...
def register_get_scheduled_wake_tool(mcp: FastMCP, cfg: Any) -> None:
    @mcp.tool()
    async def get_scheduled_wake() -> str:
        """Show the wake currently scheduled for this cloud sandbox, if
        any. Cloud (bridge) agents only."""
        if cfg.bridge_client is None:
            return "get_scheduled_wake is only available to cloud (bridge) agents."
        try:
...
def register_get_runtime_status_tool(mcp: FastMCP, cfg: Any) -> None:
    @mcp.tool()
    async def get_runtime_status() -> str:
        ...
        after a sleep."""
        if cfg.bridge_client is None:
            return "get_runtime_status is only available to cloud (bridge) agents."
        try:
...
def register_keep_alive_tool(mcp: FastMCP, cfg: Any) -> None:
    @mcp.tool()
