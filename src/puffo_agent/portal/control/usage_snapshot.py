...
def drained_harnesses(snapshot: dict) -> dict[str, int | None]:
    """``{harness: resets_at}`` for every harness whose session OR weekly
    budget is fully spent. ``resets_at`` is the soonest known reset among
    the spent windows, or ``None`` when the provider didn't give one.

    Weekly counts as drained even though the session window may still
    have room: the provider refuses turns either way, and reporting only
    the session window would leave an agent looking healthy while it
    can't answer."""
...
def apply_drained_health(snapshot: dict) -> None:
    """Drive ``runtime.health`` from the usage snapshot: flip agents on a
    spent harness to ``drained``, and clear the ones whose harness came
    back under 100%.

    This is the state-reporting half only — it runs in the control loop,
    which has no chat client, so the operator DM stays with the worker
    (which knows the agent that actually hit the wall). Other red states
    are left alone: a snapshot must not paper over auth_failed.
    """
