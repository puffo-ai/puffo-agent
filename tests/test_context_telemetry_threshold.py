"""Auto-compact threshold and context telemetry tests."""
...
def test_default_threshold_is_documented_estimate():
    assert DEFAULT_COMPACT_THRESHOLD_PCT == 95.0
    assert estimate_compact_threshold_tokens(200_000, None) == 190_000
    assert estimate_compact_threshold_tokens(1_000_000, None) == 950_000
...
def test_pct_100_estimate_does_not_claim_compaction_is_disabled():
    assert estimate_compact_threshold_tokens(
        200_000, 100
    ) == estimate_compact_threshold_tokens(200_000, None)
...
def test_payload_reports_default_when_no_override():
    ...
    assert out["auto_compact_threshold"] == 950_000
    assert out["auto_compact_threshold_pct"] is None
    assert out["threshold_is_default"] is True
    assert out["threshold_is_estimate"] is True
    assert out["used_pct"] == 13.8
...
def test_payload_reports_active_override():
    ...
    assert out["auto_compact_threshold"] == 500_000
...
def test_default_band_clears_the_override():
    for window in (200_000, 1_000_000):
        ...
        assert cleared == estimate_compact_threshold_tokens(
            window, None
        )
...
def test_docker_session_receives_overrides(tmp_path, monkeypatch):
    ...
    assert adapter.context_telemetry_env == adapter.env_overrides
    ...
    telemetry = build_context_telemetry(
        ...
        env=adapter.context_telemetry_env,
    )
    assert telemetry["max_context"] == 200_000
...
def test_completed_turn_emits_estimated_context_telemetry(tmp_path, monkeypatch):
    import asyncio
    ...
    class _Adapter(Adapter):
        ...
        async def run_turn(self, ctx: TurnContext) -> TurnResult:
            return TurnResult(
                reply="[SILENT]", metadata={"context_tokens": 80_000}
            )
    ...
    asyncio.run(drive())
    event = next(item for item in captured.events if item[1] == "context_telemetry")
    assert event[0] == "ctx-bot"
    assert event[2]["current_context"] == 80_000
    assert event[2]["auto_compact_threshold_pct"] == 50.0
    assert event[2]["threshold_is_estimate"] is True
