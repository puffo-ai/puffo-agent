...
#: Marker the platform writes into the briefing it regenerates from the profile.
_MANAGED_MARKER = b"<!-- puffo:managed-profile -->"
    ...
    if mem.is_dir():
        ...
        for f in sorted(mem.rglob("*")):
            ...
            if any(part in _NOT_CONTENT for part in rel.parts) or f.name.startswith("._"):
                continue
            data = f.read_bytes()
            if rel.parts[0] == "briefing" and _MANAGED_MARKER in data[:200]:
                # The platform-managed briefing names the SOURCE agent ("You are
                # OptionExpert (agent optionexpe-9840-…)") and is regenerated by
                # the platform from the store-owned profile. Seeding it hands the
                # new agent a stale identity — the live demo agent read it and
                # concluded it was the old one. Excluded, not rewritten: it is a
                # duplicate of something the store already owns.
                continue
